          ...
          <div className="flex items-center">
            <label className="flex items-center" htmlFor="protoFiles">
              ...
              <Tooltip
                ...
                html={t('Keep your proto files within the collection folder or the corresponding git repository to ensure paths remain valid when sharing the collection.')}
              />
            </label>
          </div>
        </div>
        ...
        <div>
          {protoFiles.some((file) => !file.exists) && (
            <div className="error-message text-xs mb-2 flex items-center p-2" data-testid="protobuf-invalid-files-message">
              ...
              {t('Some proto files cannot be found. Use the replace option to update their locations.')}
            </div>
          )}
                ...
                <tr>
                  <td colSpan="3" className="text-center">
                    <div className="empty-state flex flex-col items-center">
                      ...
                      <span className="empty-text">{t('No proto files added')}</span>
                    </div>
                  </td>
                </tr>
              ) : (
          ...
          <div className="flex items-center">
            <label className="flex items-center" htmlFor="importPaths">
              ...
              <Tooltip
                ...
                className="tooltip-mod font-normal"
                html={t('Add directories that contain proto files to be imported. These paths help resolve import statements in your proto files.')}
              />
            </label>
          </div>
        </div>
        ...
        <div>
          {importPaths.some((path) => !path.exists) && (
            <div className="error-message text-xs mb-2 flex items-center p-2" data-testid="protobuf-invalid-import-paths-message">
              <IconAlertCircle size={14} className="mr-1" />
              {t('Some import paths cannot be found at their specified locations.')}
            </div>
          )}
                ...
                <tr>
                  <td colSpan="4" className="text-center">
                    <div className="empty-state flex flex-col items-center">
                      ...
                      <span className="empty-text">{t('No import paths added')}</span>
                    </div>
                  </td>
                </tr>
              ) : (
                    ...
                    <tr key={index}>
                      <td>
                        <input
                          ...
                          className="h-4 w-4"
                          title={importPath.enabled ? t('Disable this import path') : t('Enable this import path')}
