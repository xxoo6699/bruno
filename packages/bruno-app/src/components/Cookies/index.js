...
const ClearDomainCookiesModal = ({ onClose, domain, onClear }) => {
  ...
  return (
    <Modal onClose={onClose} handleCancel={onClose} title={t('Clear Domain Cookies')} hideFooter={true}>
      ...
      <div className="font-normal mt-4">
        {t('Are you sure you want to clear all cookies for the domain {{domain}}?', { domain })}
      </div>
...
const DeleteCookieModal = ({ onClose, cookieName, onDelete }) => {
  const { t } = useTranslation();
  return (
    <Modal onClose={onClose} handleCancel={onClose} title={t('Delete Cookie')} hideFooter={true}>
      <div className="flex items-center font-normal">
        <IconAlertTriangle size={32} strokeWidth={1.5} className="warning-icon" />
        <h1 className="ml-2 text-lg font-medium">{t('Hold on..')}</h1>
      </div>
      <div className="font-normal mt-4">
        {t('Are you sure you want to delete the cookie {{name}}?', { name: cookieName })}
      </div>
...
const CollectionProperties = ({ onClose }) => {
  ...
  const clearDomainCookiesAction = () => {
    dispatch(deleteCookiesForDomain(domainToClear))
      .then(() => {
        toast.success(t('Domain cookies cleared successfully'));
      })
      .catch((err) => console.log(err) && toast.error(t('Failed to clear domain cookies')));
  ...
  const deleteCookieAction = () => {
    if (cookieToDelete) {
      ...
      dispatch(deleteCookie(domain, path, key))
        .then(() => {
          toast.success(t('Cookie deleted successfully'));
        })
        .catch((err) => console.log(err) && toast.error(t('Failed to delete cookie')));
    }
      ...
      <Modal
        ...
        customHeader={shouldShowHeader ? (
          <StyledWrapper className="header flex items-center justify-between w-full">
            <h2 className="text-xs font-medium">{t('Cookies')}</h2>
            <input
              type="search"
        ...
        <StyledWrapper>
          {!cookies || !cookies.length ? (
            ...
            <div className="flex items-center justify-center flex-col">
              ...
              <h2 className="text-lg font-medium mt-4">{t('No cookies found')}</h2>
              <p className="empty-text mt-2">{t('Add cookies to get started')}</p>
              <Button
                type="submit"
                size="sm"
                className="mt-8"
            ...
            <div className="flex items-center justify-center flex-col">
              <IconSearch size={48} />
              <h2 className="text-lg font-medium mt-4">{t('No search results')}</h2>
              <p className="empty-text mt-2">{t('Try a different search term')}</p>
            </div>
          ) : (
            // Show cookies list
                    ...
                    <Accordion.Header index={i} className="flex items-center">
                      <div className="flex items-center">
                        ...
                        <span className="domain-count ml-2 text-xs">
                          {t('({{count}} cookies)', { count: domainWithCookies.cookies.length })}
                        </span>
                        ...
                        <table className="w-full">
                          <thead>
                            <tr className="text-left">
                              <th className="py-2 px-4 font-medium w-32">{t('Name')}</th>
                              <th className="py-2 px-4 font-medium w-52">{t('Value')}</th>
                              <th className="py-2 px-4 font-medium">{t('Path')}</th>
                              <th className="py-2 px-4 font-medium">{t('Expires')}</th>
                              <th className="py-2 px-4 font-medium text-center">{t('Secure')}</th>
                              <th className="py-2 px-4 font-medium text-center">{t('HTTP Only')}</th>
                              <th className="py-2 px-4 font-medium text-right w-24">{t('Actions')}</th>
                            </tr>
                          </thead>
                          <tbody>
