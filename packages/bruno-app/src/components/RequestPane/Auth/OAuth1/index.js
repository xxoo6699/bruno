...
const OAuth1 = ({ item = {}, collection, request, save, updateAuth }) => {
  ...
  const handlePrivateKeyChange = (val) => {
    if (val && /^@file\(/.test(val.trim())) {
      toast.error(t('File references should be added using the "Upload File" button below'));
      return;
    }
  ...
  return (
    <StyledWrapper className="mt-2 flex w-full gap-4 flex-col">
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('Consumer Key')}</label>
      ...
      {!oauth1.signatureMethod?.startsWith('RSA-') && (
        <div className="flex items-center gap-4 w-full">
          <label className="block min-w-[140px]">{t('Consumer Secret')}</label>
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('Token')}</label>
        <div className="single-line-editor-wrapper flex-1">
          <SingleLineEditor
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('Token Secret')}</label>
        <div className="single-line-editor-wrapper flex-1 flex items-center">
          <SingleLineEditor
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('Signature Method')}</label>
      ...
      {oauth1.signatureMethod?.startsWith('RSA-') && (
        <div className="flex items-start gap-4 w-full">
          <label className="block min-w-[140px] mt-1">{t('Private Key')}</label>
          {isFileRef ? (
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('Add Params To')}</label>
        <div className="inline-flex items-center cursor-pointer oauth1-dropdown-selector">
          <MenuDropdown
          ...
          >
            <div className="flex items-center justify-end oauth1-dropdown-label select-none">
              {t(placementLabels[oauth1.placement] || 'Header')}
              <IconCaretDown className="caret ml-1 mr-1" size={14} strokeWidth={2} />
            </div>
          </MenuDropdown>
        </div>
      ...
      {oauth1.placement === 'body' && (
        <div className="flex items-center gap-4 w-full">
          ...
          <span className="text-xs opacity-60">
            {t('Body placement requires a form-urlencoded body. Non-form payloads will be replaced with OAuth parameters.')}
          </span>
        </div>
      )}
      ...
      {advancedOpen && (
        <>
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('Verifier')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('Timestamp')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('Nonce')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
                value={oauth1.nonce || ''}
                theme={storedTheme}
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('Version')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('Realm')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
                value={oauth1.realm || ''}
                theme={storedTheme}
