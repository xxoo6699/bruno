  ...
  return (
    <StyledWrapper className="mt-4 oauth2-additional-params-wrapper">
      ...
      <div className="tabs flex w-full gap-2 my-2">
        {availableTabs.includes('authorization') && renderTab('authorization', t('Authorization'))}
        {availableTabs.includes('token') && renderTab('token', t('Token'))}
        {availableTabs.includes('refresh') && renderTab('refresh', t('Refresh'))}
      </div>
      <Table
        headers={[
          { name: t('Key'), accessor: 'name', width: '30%' },
          { name: t('Value'), accessor: 'value', width: '30%' },
          { name: t('Send In'), accessor: 'sendIn', width: '150px' },
      ...
      >
        ...
        <span className="ml-1 text-gray-500">{t('Add Parameter')}</span>
      </div>
    </StyledWrapper>
  );
};
