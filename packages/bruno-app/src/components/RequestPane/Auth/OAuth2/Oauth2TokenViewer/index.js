    ...
    <div className="mb-2 border dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      {isExpanded && (
        <div className="p-3">
          ...
          {decodedToken && (
            <div className="mt-3">
              <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">{t('Decoded Payload')}</div>
...
const ExpiryTimer = ({ expiresIn }) => {
  ...
  return (
    ...
    >
      {timeLeft > 0 ? t('Expires in {{time}}', { time: formatExpiryTime(timeLeft) }) : t('Expired')}
    </div>
  );
};
  ...
  return (
    <StyledWrapper className="relative w-auto h-fit mt-2">
      {Object.keys(creds)?.length ? (
        creds?.error ? (
          <pre className="text-red-600 dark:text-red-400">{t('Error fetching token. Check network logs for more details.')}</pre>
        ) : (
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
            <TokenSection title={t('Access Token')} token={creds.access_token} />
            <TokenSection title={t('Refresh Token')} token={creds.refresh_token} />
            <TokenSection title={t('ID Token')} token={creds.id_token} />
            {(creds.token_type || creds.scope) ? (
              <div className="mt-3 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg text-xs">
                <div className="grid grid-cols-2 gap-2">
                  {creds.token_type ? (
                    <div className="flex items-center space-x-1">
                      <span className="font-medium">{t('Token Type:')}</span>
                      <span className="text-gray-600 dark:text-gray-300">{creds.token_type}</span>
                    </div>
                  ) : null}
                  {creds?.scope ? (
                    <div className="flex items-center space-x-1 min-w-0">
                      <span className="font-medium flex-shrink-0">{t('Scope:')}</span>
        ...
        <div className="text-gray-500 dark:text-gray-400">{t('No token found')}</div>
      )}
    </StyledWrapper>
  );
};
