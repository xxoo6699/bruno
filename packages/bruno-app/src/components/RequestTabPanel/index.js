...
const RequestTabPanel = () => {
    ...
    if (!instance) {
      return (
        <div className="pb-4 px-4">
          <div className="font-medium">{t('Mock server not found')}</div>
          <div className="text-sm mt-2 opacity-70">
            {t('This mock server may have been removed. Create a new one from the Mock Servers sidebar.')}
          </div>
        </div>
      );
    }
  ...
  if (!collection || !collection.uid) {
    return <div className="pb-4 px-4">{t('Collection not found!')}</div>;
  }
  ...
  const handleRun = async () => {
    ...
    if (isGrpcRequest && !request.url) {
      toast.error(t('Please enter a valid gRPC server URL'));
      return;
    }

    if (isGrpcRequest && !request.method) {
      toast.error(t('Please select a gRPC method'));
      return;
    }

    if (isWsRequest && !request.url) {
      toast.error(t('Please enter a valid WebSocket URL'));
      return;
    }
        ...
        {item.type === 'graphql-request' ? (
          <div className={`graphql-docs-explorer-container ${showGqlDocs ? '' : 'hidden'}`}>
            <DocExplorer schema={schema} ref={(r) => (docExplorerRef.current = r)}>
              <button className="mr-2" data-testid="graphql-docs-close-button" onClick={() => toggleDocs(false)} aria-label={t('Close Documentation Explorer')}>
                {'\u2715'}
              </button>
            </DocExplorer>
          </div>
