...
const CollectionHeader = ({ collection, isScratchCollection }) => {
  ...
  const displayName = isScratchCollection
    ? (currentWorkspace?.name || t('Untitled Workspace'))
    : (collection.name || t('Untitled Collection'));
  ...
  const overflowMenuItems = [
    { id: 'variables', label: t('Variables'), leftSection: IconEye, onClick: viewVariables },
    ...
    ...(!appAvailable
      ? [{ id: 'file-mode', label: collection.fileMode ? t('Switch to Code Mode') : t('Switch to File Mode'), leftSection: collection.fileMode ? IconFileOff : IconFileCode, onClick: handleFileModeClick }]
      : []),
    ...(!hasOpenApiSyncConfigured
      ? [{ id: 'openapi-sync', label: 'OpenAPI', leftSection: OpenAPISyncIcon, onClick: viewOpenApiSync }]
      : []),
    ...(isMockServerEnabled
      ? [{ id: 'mock-server', label: t('Mock Server'), leftSection: IconServer2, onClick: viewMockServer }]
      : []),
    { id: 'collection-settings', label: t('Collection Settings'), leftSection: IconSettings, onClick: viewCollectionSettings }
  ];
  ...
  const handleExportWorkspace = () => {
    ...
    dispatch(exportWorkspaceAction(uid))
      .then((result) => {
        if (!result?.canceled) {
          toast.success(t('Workspace exported successfully'));
        }
      })
      .catch((error) => {
        toast.error(error?.message || t('Error exporting workspace'));
      });
  };
  ...
  const handleSaveWorkspaceRename = () => {
    ...
    if (!trimmedName) {
      ...
      setWorkspaceNameError(t('Name is required'));
      return;
    }
    ...
    if (currentWorkspace?.isCreating) {
      dispatch(confirmWorkspaceCreation(uid, trimmedName))
        ...
        .catch((err) => {
          toast.error(err?.message || t('An error occurred while creating the workspace'));
        })
        .finally(() => {
      ...
      dispatch(renameWorkspaceAction(uid, workspaceNameInput))
        ...
        .catch((err) => {
          toast.error(err?.message || t('An error occurred while renaming the workspace'));
          setWorkspaceNameError(err?.message || t('Failed to rename workspace'));
        })
        .finally(() => {
          isSavingRef.current = false;
        });
            ...
            <div className="workspace-rename-container" ref={workspaceRenameContainerRef}>
              ...
              <div className="inline-actions">
                <button
                  ...
                  onMouseDown={(e) => e.preventDefault()}
                  title={currentWorkspace?.isCreating ? t('Create') : t('Save')}
                >
            ...
            <div className="flex flex-row justify-center items-center gap-x-1">
              ...
              >
                <div className="max-w-124 overflow-hidden">
                  {currentWorkspace && (
                    <>
                      <div className="label-item">{t('Workspace')}</div>
                      <div
                  ...
                  {mountedCollections.length > 0 && (
                    <>
                      ...
                      <div className="label-item">{t('Collections')}</div>
                      {mountedCollections.map((col) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="dropdown-label collection-header-dropdown-label">{col.name || t('Untitled Collection')}</span>
                            {colTabCount > 0 && (
            ...
            <>
              {appAvailable && (
                <div className="mode-toggle" data-testid="view-mode-toggle">
                  <ToolHint text={t('Request')} toolhintId="ViewModeRequestToolhintId" place="bottom">
                    <button
                      type="button"
                      data-testid="view-mode-request"
                      aria-label={t('Request view')}
                  ...
                  <ToolHint text={t('App')} toolhintId="ViewModeAppToolhintId" place="bottom">
                    <button
                      type="button"
                  ...
                  <ToolHint text={t('File')} toolhintId="ViewModeFileToolhintId" place="bottom">
                    <button
                      type="button"
              ...
              {isAiEnabled && (
                <ToolHint text={t('AI Assistant')} toolhintId="AiAssistantToolhintId" place="bottom">
                  <ActionIcon
              ...
              {collection.format === 'bru' && !migratePillDismissed && (
                <div
                  ...
                  title={t('Migrate this collection to YML')}
                >
                  <button
                    type="button"
                    className="pill-main"
                  ...
                  >
                    ...
                    <span className="pill-label">{t('Migrate to YML')}</span>
                  </button>
                  <button
                    type="button"
              ...
              {hasOpenApiSyncConfigured && (
                <ToolHint
                  text={hasOpenApiError ? t('OpenAPI Error') : hasOpenApiUpdates ? t('OpenAPI Updates Available') : 'OpenAPI'}
              ...
              <ToolHint text={t('Runner')} toolhintId="RunnerToolhintId" place="bottom">
                <ActionIcon onClick={handleRun} aria-label={t('Runner')} size="sm" data-testid="runner">
              ...
              <MenuDropdown items={overflowMenuItems} placement="bottom-end" data-testid="more-actions">
                <ActionIcon label={t('More actions')} size="sm" style={{ border: `1px solid ${theme.border.border1}`, borderRadius: theme.border.radius.base, width: 24, marginRight: 4, marginLeft: 4 }}>
