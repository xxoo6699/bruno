...
const ResultIcon = ({ status }) => {
  ...
  return (
    ...
    >
      {status === 'pass' ? (
        <IconCircleCheck size={14} className="mr-1" aria-label={t('Test passed')} />
      ) : (
        <IconCircleX size={14} className="mr-1" aria-label={t('Test failed')} />
      )}
    </span>
  );
};
    ...
    <div className="mb-4">
      ...
      >
        ...
        <span className="flex-grow">
          {t(title)} ({results.length}), {t('Passed')}: {passedResults.length}, {t('Failed')}: {failedResults.length}
        </span>
      </div>
      {isExpanded && (
        <ul className="ml-5">
