...
const CreateCollection = ({ onClose, defaultLocation: propDefaultLocation, initialCollectionName = '' }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      collectionName: Yup.string()
        .trim()
        .min(1, t('Collection name can\'t be empty'))
      ...
      format: Yup.string().oneOf(['bru', 'yml'], t('invalid format')).required(t('Format is required'))
    }),
    onSubmit: async (values) => {
      try {
        ...
        toast.error(multiLineMsg(t('An error occurred while creating the collection'), formatIpcError(e)));
      }
    }
  });
  ...
  return (
    <Portal>
      <StyledWrapper>
        <Modal size="md" title={t('Create Collection')} hideFooter={true} handleCancel={onClose}>
          <form className="bruno-form" onSubmit={formik.handleSubmit}>
            <div>
              ...
              <label htmlFor="collection-location" className="font-medium mt-3 flex items-center">
                ...
                <Help>
                  <p>
                    {t('Bruno stores your collections on your computer\'s filesystem.')}
                  </p>
                  <p className="mt-2">
                    {t('Choose the location where you want to store this collection.')}
                  </p>
                </Help>
              </label>
              <input
              ...
              {showAdvancedOptions && (
                <div className="mt-4">
                  <label htmlFor="format" className="flex items-center font-medium">
                    ...
                    <Help width="300">
                      <p>
                        {t('Choose the file format for storing requests in this collection.')}
                      </p>
                      <p className="mt-2">
                        <strong>{t('OpenCollection (YAML):')}</strong> {t('Industry-standard YAML format (.yml files)')}
                      </p>
                      <p className="mt-1">
                        <strong>{t('BRU:')}</strong> {t('Bruno\'s native file format (.bru files)')}
                      </p>
                    </Help>
                  </label>
                  <select
