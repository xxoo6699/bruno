...
const NewFolder = ({ collectionUid, item, onClose }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      directoryName: Yup.string()
        .trim()
        .min(1, t('must be at least 1 character'))
        .required(t('foldername is required'))
        ...
        .test({
          name: 'folderName',
          message: t('The folder name "environments" at the root of the collection is reserved in bruno'),
          test: (value) => {
    ...
    onSubmit: (values) => {
      dispatch(newFolder(values.folderName, values.directoryName, collectionUid, item ? item.uid : null))
        .then(() => {
          toast.success(t('New folder created!'));
          onClose();
        })
        .catch((err) => toast.error(err ? err.message : t('An error occurred while adding the folder')));
    }
  });
  ...
  return (
    <Portal>
      <StyledWrapper>
        <Modal size="md" title={t('New Folder')} hideFooter={true} handleCancel={onClose}>
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between">
                  <label htmlFor="directoryName" className="flex items-center font-medium">
                    {t('Folder Name')} <small className="font-normal text-muted ml-1">{t('(on filesystem)')}</small>
                    <Help width="300">
                      <p>
