...
const NewRequest = ({ collectionUid, item, isEphemeral, onClose }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      filename: Yup.string()
        ...
        .min(1, t('must be at least 1 character'))
        .max(255, t('must be 255 characters or less'))
        .required(t('filename is required'))
        ...
        .test(
          'not-reserved',
          t('The file names "collection" and "folder" are reserved in bruno'),
      ...
      curlCommand: Yup.string().when('requestType', {
        ...
        then: Yup.string()
          .min(1, t('must be at least 1 character'))
          .required(t('curlCommand is required'))
          .test({
            name: 'curlCommand',
            message: t('Invalid cURL Command'),
    ...
    onSubmit: (values) => {
      ...
      if (isGrpcRequest) {
        dispatch(
          ...
          .then(() => {
            toast.success(t('New request created!'));
            onClose();
          })
          .catch((err) => toast.error(err ? err.message : t('An error occurred while adding the request')));
  ...
  return (
    <Portal>
      <StyledWrapper>
        <Modal size="md" title={t('New Request')} hideFooter handleCancel={onClose}>
          <form
          ...
          >
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between">
                  <label htmlFor="filename" className="flex items-center font-medium">
                    {t('File Name')} <small className="font-normal text-muted ml-1">{t('(on filesystem)')}</small>
                    <Help width="300">
                      <p>{t('Bruno saves each request as a file in your collection\'s folder.')}</p>
                      <p className="mt-2">
            ...
            {formik.values.requestType !== 'from-curl' ? (
              ...
              <div className="mt-4">
                ...
                <textarea
                  name="curlCommand"
                  placeholder={t('Enter cURL request here..')}
