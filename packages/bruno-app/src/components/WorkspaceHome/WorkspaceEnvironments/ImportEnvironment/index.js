...
const ImportEnvironment = ({ onClose, onEnvironmentCreated }) => {
  ...
  const handleImportPostmanEnvironment = () => {
    importPostmanEnvironment()
        ...
        const importPromises = environments
          ...
          .map((environment) =>
            dispatch(addGlobalEnvironment({ name: environment.name, variables: environment.variables }))
              .then(() => {
                toast.success(t('Environment imported successfully'));
              })
              .catch((error) => {
                toast.error(t('An error occurred while importing the environment'));
                console.error(error);
              }));
      ...
      .catch((err) => toastError(err, t('Postman Import environment failed')));
  };

  return (
    <Portal>
      <Modal size="sm" title={t('Import Environment')} hideFooter={true} handleConfirm={onClose} handleCancel={onClose} dataTestId="import-environment-modal">
        <button
          type="button"
        ...
        >
          ...
          <span className="mt-2 block text-sm font-semibold">{t('Import your Postman environments')}</span>
        </button>
      </Modal>
    </Portal>
  );
